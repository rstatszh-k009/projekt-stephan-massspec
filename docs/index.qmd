...
Gibt es Unterschiede zwischen einzelnen Matrizes?
...
Weiter konnte auch gezeigt werden das Gehalte zwischen den Matrizes sich stark unterscheiden (siehe @matrix_box_cort und @matrix_box_hydrocort). Es fällt auf das Cortison vorwiegend in der Matrix Niere und Harn vorkommt wärend Hydrocortison in allen Matrizes zu finden ist wobei die Gehalte in der Leber tiefer waren.

Zwischen den Tierarten wurde nur die Matrix Muskel verglichen da von den meisten Tieren nur diese Matrix beprobt wurde (siehe @tierart_box_cort und @tierart_box_hydrocort). Dabei hat sich gezeigt das bei Schweinen und Fischen höhere Mengen an Cortison gefunden werden, wärend bei Tieren der Rindergattung keine Werte über der Bestimmungsgrenze gemessen wurden. In 99 % aller Muskelproben wurde hydrocortison nachgewiesen (siehe @Tbl_Hydrocortison_Matrix).

### Matrix

Es wurden nur die Daten von Tieren der Rindergattung und Schweinen für die Visualisiernug verwendet, da von den anderen Tierarten nicht alle Matrizes beprobt wurden.
...
Eine Jahreszeitliche Abhängigkeit der Hydrocortison Werte konnte nicht eindutig gezeigt werden (siehe @jahreszeit_box_muskel und @jahreszeit_box_harn). Die gefundenen Unterschiede sind eher gering und von den Schweinen wurden keine Harnproben im Herbst durchgeführt.

### Cortison und Hydrocortison als Methodenkontrolle

Aufgrund des natürliche Vorkommens von Cortison und Hydrocortison bietet sich die Möglichkeit diese Substanzen für die Kontrolle der Analysemethode zu verwenden.

In Muskelproben wurden in 99% aller Proben Hydrocortison über der Bestimmungsgrenze gemessen. Muskelproben welche kein Hydrocortison enthalten sollten genau geprüft und gegebenenfalls nochmals gemessen werden.
