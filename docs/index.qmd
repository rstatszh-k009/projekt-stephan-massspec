...
Mit der folgenden Datenanalyse soll gezeit werden ob sich von den gemessenen Konzentrationen ein Beurteilungskriterium ableiten lässt oder ob die Substanzen als positiv kontrolle für die Methode verwendet werden können.
...
```{r}
leber <- filter(cortison_tidy, matrix == "Leber")
niere <- filter(cortison_tidy, matrix == "Niere")
muskel <- filter(cortison_tidy, matrix == "Muskel")
```
