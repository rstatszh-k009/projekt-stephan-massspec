...
ohne_NA |> 
  group_by(matrix) |> 
  summarise("Anzahl Proben" = n(),"Prozent negativ" = round((sum(cortison_pos/n()))*100,0),"Mittelwert [µg/kg]" = round(mean(cortison_low),0),"Standard Abweichung" = round(sd(cortison_low),0),"höchster Wert [µg/kg]" = max(cortison_low)) |> 
  gt()
